...
- Public Nitro hook augmentations that consumers should get from `import 'c8y-nitro'` must live in the root entrypoint type graph
  ...
  - For lifecycle hooks on cached data, emit only when the effective payload actually changes; compare normalized content, not just item counts, and treat ordering as irrelevant
