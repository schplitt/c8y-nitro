/**
 * Generated by tsnapi — public API snapshot of `c8y-nitro/types`
 */
// #region Interfaces
export interface C8YAPIClientOptions {
  dir: string;
  contextPath?: string;
}
export interface C8yCacheOptions {
  credentialsTTL?: number;
  defaultTenantOptionsTTL?: number;
  tenantOptions?: C8YTenantOptionKeysCacheConfig;
}
export interface C8yDevOptions {
  injectUser?: boolean;
}
export interface C8yDockerOptions {
  dockerfile?: string;
  baseImage?: string;
  extraInstructions?: string[];
}
export interface C8YManifest {
  apiVersion: string;
  key: string;
  type: 'MICROSERVICE';
  name: string;
  version: string;
  contextPath?: string;
  provider: Provider;
  billingMode?: 'RESOURCES' | 'SUBSCRIPTION';
  isolation?: 'MULTI_TENANT' | 'PER_TENANT';
  scale?: 'AUTO' | 'NONE';
  replicas?: number;
  resources?: Resources;
  requestedResources?: RequestedResources;
  settings?: Option[];
  settingsCategory?: string;
  requiredRoles?: C8YRole$1[];
  roles?: string[];
  livenessProbe?: Probe;
  readinessProbe?: Probe;
  extensions?: Extension[];
  openApiSpec?: string;
}
export interface C8yNitroModuleOptions {
  dev?: C8yDevOptions;
  manifest?: C8YManifestOptions;
  apiClient?: C8YAPIClientOptions;
  zip?: C8YZipOptions;
  docker?: C8yDockerOptions;
  cache?: C8yCacheOptions;
  realtime?: C8yRealtimeOptions;
  openapi?: C8yOpenAPIOptions;
  enableTenantOptionsInvalidationRoute?: boolean;
  skipBootstrap?: boolean;
}
export interface C8yOpenAPIOptions {
  excludeInternalRoutes?: boolean;
  excludeRoutes?: string[];
}
export interface C8yRealtimeOptions {
  name?: string;
  deleteSubscriptionsOnClose?: boolean;
  deleteSubscriptionOnEmpty?: boolean;
  autoAck?: boolean;
  dedupe?: boolean;
  autoStart?: boolean;
  subscription?: {
    apis?: SubscriptionApi[];
    nonPersistent?: boolean;
  };
  resilience?: ConsumerResilienceOptions;
}
export interface C8YRoles {}
export interface C8YZipOptions {
  name?: string | ((_: string, _: string) => string);
  outputDir?: string;
  manifest?: C8YManifestOptions;
}
// #endregion

// #region Types
export type C8YKnownRole = 'ROLE_ALARM_ADMIN' | 'ROLE_ALARM_READ' | 'ROLE_APPLICATION_MANAGEMENT_ADMIN' | 'ROLE_APPLICATION_MANAGEMENT_READ' | 'ROLE_AUDIT_ADMIN' | 'ROLE_AUDIT_READ' | 'ROLE_BINARY_ADMIN' | 'ROLE_BINARY_CREATE' | 'ROLE_BINARY_READ' | 'ROLE_BULK_OPERATION_ADMIN' | 'ROLE_BULK_OPERATION_READ' | 'ROLE_DEVICE' | 'ROLE_DEVICE_BOOTSTRAP' | 'ROLE_DEVICE_CONTROL_ADMIN' | 'ROLE_DEVICE_CONTROL_READ' | 'ROLE_EVENT_ADMIN' | 'ROLE_EVENT_READ' | 'ROLE_IDENTITY_ADMIN' | 'ROLE_IDENTITY_READ' | 'ROLE_INVENTORY_ADMIN' | 'ROLE_INVENTORY_CREATE' | 'ROLE_INVENTORY_READ' | 'ROLE_MANAGED_OBJECT_ADMIN' | 'ROLE_MANAGED_OBJECT_CREATE' | 'ROLE_MANAGED_OBJECT_READ' | 'ROLE_MEASUREMENT_ADMIN' | 'ROLE_MEASUREMENT_READ' | 'ROLE_NOTIFICATION_2_ADMIN' | 'ROLE_OPTION_MANAGEMENT_ADMIN' | 'ROLE_OPTION_MANAGEMENT_READ' | 'ROLE_RETENTION_RULE_ADMIN' | 'ROLE_RETENTION_RULE_READ' | 'ROLE_SMS_READ' | 'ROLE_SYSTEM' | 'ROLE_TENANT_ADMIN' | 'ROLE_TENANT_ALARM_ADMIN' | 'ROLE_TENANT_MANAGEMENT_ADMIN' | 'ROLE_TENANT_MANAGEMENT_CREATE' | 'ROLE_TENANT_MANAGEMENT_READ' | 'ROLE_TENANT_MANAGEMENT_UPDATE' | 'ROLE_TENANT_STATISTICS_READ' | 'ROLE_USER_MANAGEMENT_ADMIN' | 'ROLE_USER_MANAGEMENT_CREATE' | 'ROLE_USER_MANAGEMENT_OWN_ADMIN' | 'ROLE_USER_MANAGEMENT_OWN_READ' | 'ROLE_USER_MANAGEMENT_READ';
export type C8YManifestOptions = Omit<C8YManifest, 'name' | 'version' | 'apiVersion' | 'key' | 'type' | 'provider' | 'openApiSpec'>;
export type C8YRole = C8YKnownRole | (string & {});
export type C8YSettingsCategory = string;
export type C8YTenantOptionKey = string;
export type C8YTenantOptionKeysCacheConfig = Partial<Record<C8YTenantOptionKey$1, number>>;
export type TenantCredentials = Record<string, ICredentials>;
// #endregion