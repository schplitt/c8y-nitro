...
function getReturnType(handlerPath: string, outputFile: string): string {
  ...
  // Return full type with C8ySerialize wrapper that handles:
  // - void/undefined/null -> undefined (empty body)
  // - string -> string (text/plain)
  // - everything else -> JSON serialized
  return `C8ySerialize<Awaited<ReturnType<typeof import('${importPathWithoutExt}').default>>>`
}
